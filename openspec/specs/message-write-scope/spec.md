...
系统 MUST 仅允许在 `rag_retrieve`、`build_question_pool`、`generate_answer`、`eval_answer` 四个节点向 `state["messages"]` 写入消息。

#### Scenario: 允许的节点写入消息
- **WHEN** `rag_retrieve` 或 `build_question_pool` 或 `generate_answer` 或 `eval_answer` 节点执行完成
