# graph-error-propagation Specification

## Purpose
TBD - created by archiving change fix-multi-agent-graph-issues. Update Purpose after archive.
...
系统 SHALL 在 `correct_context=False` 时触发重试，并使用 `retrieved_epoch` 记录重试次数。

#### Scenario: 问题池为空触发重试

- **WHEN** `correct_context=False`
- **THEN** 系统 SHALL 增加 `retrieved_epoch` 计数 +1
- **AND** SHALL 路由回检索节点重试
...
系统 SHALL 在初始化状态时设置 `retrieved_epoch=0`。
