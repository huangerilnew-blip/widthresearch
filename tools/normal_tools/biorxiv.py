...
class BioRxivSearcher(PaperSource):
    ...
    def __init__(self):
        self.session = httpx.AsyncClient() ##verify=False 跳过证书验证
    ...
    async def download_pdf(self, paper_id: str, save_path: str) -> str:
        ...
        pdf_url = f"https://www.biorxiv.org/content/{paper_id}v{self.max_retries}.full.pdf"
        tries = 0
        while tries < self.max_retries:
            try:
                ...
                response = await self.session.get(pdf_url, timeout=self.timeout, headers=headers)
                response.raise_for_status()
